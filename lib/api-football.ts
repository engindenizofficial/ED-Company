...
export async function getPlayerPageData(playerId: number, season?: number): Promise<PlayerPageData | null> {
  // Use current season if not specified
  const currentSeason = season ?? new Date().getFullYear() - (new Date().getMonth() < 7 ? 1 : 0)

  const [statsRaw, transfersRaw, tropicsRaw] = await Promise.all([
    safeFetch<any>("/players", { id: playerId, season: currentSeason }, 3600),
    safeFetch<any>("/transfers", { player: playerId }, 3600),
    safeFetch<any>("/trophies", { player: playerId }, 3600),
  ])

  if (statsRaw.length === 0) return null

  const first = statsRaw[0]
  ...
  const profile: PlayerProfile = {
    ...
    photo: p.photo ?? null,
    position: first.statistics?.[0]?.games?.position ?? null,
    number: first.statistics?.[0]?.games?.number ?? null,
    injured: p.injured ?? false,
    team: first.statistics?.[0]?.team
      ? { id: first.statistics[0].team.id, name: first.statistics[0].team.name, logo: first.statistics[0].team.logo }
      : null,
    league: first.statistics?.[0]?.league
      ? {
          id: first.statistics[0].league.id,
          name: first.statistics[0].league.name,
          country: first.statistics[0].league.country,
          logo: first.statistics[0].league.logo,
          season: first.statistics[0].league.season,
        }
      : null,
  }

  const stats: PlayerSeasonStats[] = (first.statistics ?? []).map((s: any) => ({
    season: s.league?.season ?? currentSeason,
    team: { id: s.team?.id ?? 0, name: s.team?.name ?? "", logo: s.team?.logo ?? "" },
    league: { id: s.league?.id ?? 0, name: s.league?.name ?? "", country: s.league?.country ?? "", logo: s.league?.logo ?? "" },
    ...
    dribblesAttempted: s.dribbles?.attempts ?? null,
    dribblesSuccess: s.dribbles?.success ?? null,
  }))

  const transfers: Transfer[] = (transfersRaw ?? []).flatMap((t: any) =>
    (t.transfers ?? []).map((tr: any) => ({
      date: tr.date ?? null,
      type: tr.type ?? "",
      teamFrom: { id: t.teams?.out?.id ?? 0, name: t.teams?.out?.name ?? "", logo: t.teams?.out?.logo ?? "" },
      teamTo: { id: t.teams?.in?.id ?? 0, name: t.teams?.in?.name ?? "", logo: t.teams?.in?.logo ?? "" },
    }))
  )

  const trophies: Trophy[] = (tropicsRaw ?? []).map((t: any) => ({
    league: t.league ?? "",
    country: t.country ?? "",
    season: t.season ?? "",
    place: t.place ?? "",
  ...
  return {
    profile,
    stats,
    transfers: transfers.sort((a, b) => (b.date ?? "").localeCompare(a.date ?? "")),
    trophies,
    cachedAt: Date.now(),
  }
}
...
export async function getLeaguePageData(leagueId: number, season?: number): Promise<LeaguePageData | null> {
  const currentSeason = season ?? new Date().getFullYear() - (new Date().getMonth() < 7 ? 1 : 0)

  const [standingsRaw, scorersRaw, fixturesRaw, leagueInfoRaw] = await Promise.all([
    safeFetch<any>("/standings", { league: leagueId, season: currentSeason }, 3600),
    safeFetch<any>("/players/topscorers", { league: leagueId, season: currentSeason }, 3600),
    safeFetch<RawFixture>("/fixtures", { league: leagueId, season: currentSeason, last: 20, next: 10 }, 1800),
    safeFetch<any>("/leagues", { id: leagueId, season: currentSeason }, 3600),
  ])

  const leagueInfo = leagueInfoRaw[0]
  ...
  const groups: any[][] = standingsRaw[0]?.league?.standings ?? []
  ...
  const topScorers: TopScorer[] = (scorersRaw ?? []).map((s: any) => ({
    player: {
      id: s.player?.id ?? 0,
      name: s.player?.name ?? "",
    ...
    team: { id: s.statistics?.[0]?.team?.id ?? 0, name: s.statistics?.[0]?.team?.name ?? "", logo: s.statistics?.[0]?.team?.logo ?? "" },
    goals: s.statistics?.[0]?.goals?.total ?? null,
    assists: s.statistics?.[0]?.goals?.assists ?? null,
    yellowCards: s.statistics?.[0]?.cards?.yellow ?? null,
    redCards: s.statistics?.[0]?.cards?.red ?? null,
    appearances: s.statistics?.[0]?.games?.appearences ?? null,
    rating: s.statistics?.[0]?.games?.rating ?? null,
  }))

  const fixtures = fixturesRaw.map(mapFixture).sort((a, b) => b.timestamp - a.timestamp)
