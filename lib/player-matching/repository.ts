...
export async function saveMatchBatch(runId: string, decisions: MatchDecision[]) {
  ...
  try {
    ...
    const exactMatches = decisions.filter((item) => item.level === 'exact_biographic').length
    const fuzzyMatches = decisions.filter((item) => item.level === 'fuzzy_name_birthdate').length
    const unmatched = decisions.filter((item) => item.level === 'unmatched').length
    await client.query(`UPDATE player_match_run SET "processedPlayers"="processedPlayers"+$2,"exactMatches"="exactMatches"+$3,"fuzzyMatches"="fuzzyMatches"+$4,"unmatchedPlayers"="unmatchedPlayers"+$5,"activePlayer"=$6,"heartbeatAt"=now(),"updatedAt"=now() WHERE id=$1`, [runId, decisions.length, exactMatches, fuzzyMatches, unmatched, decisions.at(-1)?.transfermarktPlayer.name])
