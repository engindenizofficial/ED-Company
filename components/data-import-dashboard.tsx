...
type DashboardData = { available: boolean; message?: string; runs: Partial<Record<ImportSource, Run>>; checkpoints: Checkpoint[]; errors: ImportError[] }
...
function SourceCard({ source, run, onStart, busy }: { source: ImportSource; run?: Run; onStart: (source: ImportSource) => void; busy: boolean }) {
  const [phrase, setPhrase] = useState('')
  const progress = run ? Math.min(100, Math.round((run.processedLeagues / Math.max(1, run.totalLeagues)) * 100)) : 0
  const stale = run?.status === 'running' && Date.now() - new Date(run.heartbeatAt).getTime() > 180_000
...
export function DataImportDashboard() {
  ...
  return <div className="flex flex-col gap-6">
    ...
    <div className="grid gap-4 xl:grid-cols-2"><SourceCard source="transfermarkt" run={data?.runs.transfermarkt} onStart={start} busy={busy} /><SourceCard source="api_football" run={data?.runs.api_football} onStart={start} busy={busy} /></div>
