...
type MatchingRun = { id: string; status: string; stage: string; totalPlayers: number; processedPlayers: number; exactMatches: number; fuzzyMatches: number; unmatchedPlayers: number; errorCount: number; activePlayer?: string | null; errorMessage?: string | null; heartbeatAt: string; startedAt: string; finishedAt?: string | null }
type DashboardData = { available: boolean; message?: string; serverNow: string; runs: Partial<Record<ImportSource, Run>>; checkpoints: Checkpoint[]; errors: ImportError[]; summaries: Record<string, RunSummary>; matching?: MatchingRun | null }
...
function MatchingCard({ run, onStart, busy, sourcesReady }: { run?: MatchingRun | null; onStart: () => void; busy: boolean; sourcesReady: boolean }) {
  const progress = run ? Math.min(100, Math.round((run.processedPlayers / Math.max(1, run.totalPlayers)) * 100)) : 0
  ...
  return <Card className="xl:col-span-2">
    ...
    <CardFooter><Button onClick={onStart} disabled={busy || active || !sourcesReady}><Play data-icon="inline-start" />{run ? 'Yeniden eşleştir' : 'Eşleştirmeyi başlat'}</Button></CardFooter>
  </Card>
}
...
export function DataImportDashboard() {
  ...
  async function startMatching() {
    setBusy(true)
    try {
      const response = await fetch('/api/admin/data-import/matching/start', { method: 'POST' })
      const body = await response.json().catch(() => ({}))
      if (!response.ok) throw new Error(body.message)
      toast.success('Oyuncu eşleştirme koşusu başlatıldı.')
      await mutate()
    } catch (error) {
      toast.error(error instanceof Error && error.message ? error.message : 'Eşleştirme başlatılamadı.')
    } finally { setBusy(false) }
  }
  const sourcesReady = data?.runs.transfermarkt?.status === 'completed' && data?.runs.api_football?.status === 'completed'
