...
type ImportError = { id: string; runId: string; kind: string; itemKey?: string | null; errorType: string; message: string; retryable: boolean; createdAt: string }
type DashboardData = { available: boolean; message?: string; serverNow: string; runs: Partial<Record<ImportSource, Run>>; checkpoints: Checkpoint[]; errors: ImportError[] }
...
function SourceCard({ source, run, onStart, busy, serverNow }: { source: ImportSource; run?: Run; onStart: (source: ImportSource) => void; busy: boolean; serverNow?: string }) {
  ...
  return <Card>
    ...
    <CardContent className="flex flex-col gap-5">
      <div className="flex flex-col gap-2"><div className="flex items-center justify-between text-sm"><span>{run?.stage ?? 'Henüz başlatılmadı'}</span><strong>{run?.processedLeagues ?? 0}/23 lig</strong></div><Progress value={progress} aria-label={`${sourceLabels[source]} lig ilerlemesi`} /></div>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[['Takımlar', `${run?.processedTeams ?? 0}/${run?.totalTeams ?? 0}`], ['Oyuncular', `${run?.processedPlayers ?? 0}/${run?.totalPlayers ?? 0}`], ['Hata', String((run?.failedTeams ?? 0) + (run?.failedPlayers ?? 0))], ['Restart', String(run?.restartCount ?? 0)]].map(([label, value]) => <div key={label} className="rounded-lg bg-muted p-3"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 font-mono text-lg font-semibold">{value}</p></div>)}
      </div>
    ...
    <CardFooter className="justify-between gap-2">
      <Button onClick={() => onStart(source)} disabled={busy || ['queued','running'].includes(run?.status ?? '')}><Play data-icon="inline-start" />{run ? 'Devam ettir' : 'Başlat'}</Button>
      <AlertDialog>
...
export function DataImportDashboard() {
  ...
  return <div className="flex flex-col gap-6">
    ...
    <div className="grid gap-4 xl:grid-cols-2"><SourceCard source="transfermarkt" run={data?.runs.transfermarkt} onStart={start} busy={busy} serverNow={data?.serverNow} /><SourceCard source="api_football" run={data?.runs.api_football} onStart={start} busy={busy} serverNow={data?.serverNow} /></div>
    <Tabs defaultValue="progress"><TabsList><TabsTrigger value="progress">Lig ve takım ilerlemesi</TabsTrigger><TabsTrigger value="errors">Hata günlüğü</TabsTrigger></TabsList>
      ...
      <TabsContent value="errors"><Card><CardHeader><CardTitle>Hata günlüğü</CardTitle><CardDescription>Koruma, rate limit, parser ve yeniden başlatma hataları.</CardDescription></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Tür</TableHead><TableHead>Aşama</TableHead><TableHead>Mesaj</TableHead><TableHead>Zaman</TableHead></TableRow></TableHeader><TableBody>{(data?.errors ?? []).map((item) => <TableRow key={item.id}><TableCell><Badge variant="destructive">{item.errorType}</Badge></TableCell><TableCell>{item.kind}</TableCell><TableCell className="max-w-md whitespace-normal">{item.message}</TableCell><TableCell>{date(item.createdAt)}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card></TabsContent>
    </Tabs>
  </div>
}
